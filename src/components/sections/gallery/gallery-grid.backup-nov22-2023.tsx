...
export function GalleryGrid(): React.JSX.Element {
  ...
  // Move handleGridKeyDown outside of the map function
  ...
  const setSliderRef = (el: HTMLDivElement | null, index: number) => {
    if (sliderRefs.current.length <= index) {
      sliderRefs.current = [...sliderRefs.current, ...new Array(index - sliderRefs.current.length + 1).fill(null)]
    }
            ...
            >
              {filteredItems.map((item, index) => {
                ...
                const { 
                  ...
                  containerRef: { current: sliderRefs.current[index] }
                })
